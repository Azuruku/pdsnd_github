...
def get_city():
    ...
    city = input('\nHello! Let\'s explore some US bikeshare data!\n'
                 'Would you love to view data for Chicago, New York, or Washington?\n').title()
    if city == 'Chicago':
        return 'chicago.csv'
    elif city == 'New York':
...
def get_time_period():
    ...
    '''
    time_period = input('\nWould you love to filter the data by month, day, or not at'
...
def display_data(df, current_line):
    '''
    Returns:
        If the user says yes then this function returns the next ten lines
    ...
    if display == 'yes' or display == 'y':
        print(df.iloc[current_line:current_line+10])
        current_line += 10
